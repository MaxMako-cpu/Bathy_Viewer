...
# genuinely shorter.
# Hiding a chain must not change how the wire is read. It did once: the
# buttons were wired to the decoder's record length, so deselecting a
# *deployed* ROV to clear the view told the decoder it had stopped
# transmitting. One datagram then read as three vessel fixes 130 m apart -
# the vessel parked on the TMS, and the trail zigzagging between all three.
check("hiding a chain does not change how the feed is decoded",
      win.feed.order == ORDER and win.dfeed.order == DEPTH_ORDER,
      f"{win.feed.order} / {win.dfeed.order}")
check("nor the expected record length",
      win.feed.fields == 10 and win.dfeed.fields == 4,
      f"{win.feed.fields} / {win.dfeed.fields}")
check("and the hidden chain keeps its last known fix",
      "ROV2" in win._positions, str(sorted(win._positions)))
