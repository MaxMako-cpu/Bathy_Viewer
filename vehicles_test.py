...
from bathy3d.feed import ORDER, TETHERS, slot_for   # noqa: E402
...
check("a slot starts out labelled with its own name",
      f.label("ROV1") == "ROV1")
check("and coloured as targets.py ships it", f.colour("ROV1") == "#ff3b30",
      f.colour("ROV1"))

f.set_label("ROV1", "Hercules")
check("a rename takes", f.label("ROV1") == "Hercules")
check("and is noticed", f.renamed())
check("the other bodies are untouched", f.label("ROV2") == "ROV2")

f.set_label("ROV1", "   Deep   Rover  ")
check("whitespace is tidied", f.label("ROV1") == "Deep Rover",
      repr(f.label("ROV1")))
f.set_label("ROV1", "x" * 60)
check("an absurd name is capped", len(f.label("ROV1")) == 24,
      str(len(f.label("ROV1"))))
f.set_label("ROV1", "")
check("blank puts the slot name back", f.label("ROV1") == "ROV1")
check("and the fleet is stock again", not f.renamed())

# A TMS has no colour of its own, so there is nothing that can drift.
f.set_colour("ROV2", "#3366ff")
check("an ROV takes a new colour", f.colour("ROV2") == "#3366ff")
check("its TMS follows it, darker",
      f.colour("TMS2") == vehicles.darken("#3366ff"), f.colour("TMS2"))
check("the darker shade really is darker",
      vehicles.darken("#3366ff") != "#3366ff")
try:
    f.set_colour("TMS2", "#ffffff")
    refused = False
except ValueError:
    refused = True
check("a TMS colour cannot be set directly", refused)
check("the TMS still follows its ROV",
      f.colour("TMS2") == vehicles.darken("#3366ff"))

check("only the vessel and the ROVs are choosable",
      vehicles.CHOOSABLE == ("Vessel", "ROV1", "ROV2"),
...
check("a nonsense colour is ignored rather than stored",
      (f.set_colour("ROV2", "not a colour") or f.colour("ROV2")) == "#3366ff")
...
check("labels and colours survive the round trip",
      ...
      and back.colour("ROV2") == "#3366ff"
      and back.colour("TMS2") == f.colour("TMS2"), str(rows))
check("no TMS colour is stored, so it cannot go stale",
      not any(r.startswith("TMS") and r.split("|")[2] for r in rows), str(rows))
bad = vehicles.Fleet()
bad.load(["rubbish", "NoSuchBody|Name|#ffffff", "ROV1|Keeper|"])
check("unreadable rows are dropped, good ones kept",
      bad.label("ROV1") == "Keeper" and bad.encode() == ["ROV1|Keeper|"],
      str(bad.encode()))

# The slots used to be one vessel's own vehicle names. Settings written then -
# and calibration tie-ins especially, which outlive a session - still carry
# them, so they are mapped forward rather than silently dropped.
check("legacy slot names map forward",
      slot_for("UHD333") == "ROV1" and slot_for("UHD334") == "ROV2"
      and slot_for("TMS333") == "TMS1" and slot_for("TMS334") == "TMS2")
check("a current slot passes through untouched", slot_for("ROV1") == "ROV1")
old = vehicles.Fleet()
old.load(["UHD333|Hercules|#00a3ff", "TMS334||"])
check("a fleet saved under the old slots still loads",
      old.label("ROV1") == "Hercules" and old.colour("ROV1") == "#00a3ff",
      str(old.encode()))
check("and re-encodes under the new ones",
      old.encode() == ["ROV1|Hercules|#00a3ff"], str(old.encode()))
...
for _ in range(40):
    pump(80)
    if len(win.view.targets.targets) == len(ORDER) and win._depths.get("ROV1"):
        break
...
check("the table starts with the slot name",
      win.tgt_table.item(row, 0).text() == "ROV1")
check("the tie-in menu names it too",
      "ROV1" in win.act_tie["ROV1"].text(), win.act_tie["ROV1"].text())
...
win.fleet.set_colour("ROV1", "#00a3ff")
win.apply_fleet()
pump(200)

check("the marker carries the new label",
      win.view.targets.targets["ROV1"].shown == "Hercules",
      win.view.targets.targets["ROV1"].shown)
check("and the new colour",
      win.view.targets.targets["ROV1"].color == "#00a3ff",
      win.view.targets.targets["ROV1"].color)
check("its TMS was recoloured to match",
      win.view.targets.targets["TMS1"].color == vehicles.darken("#00a3ff"),
      win.view.targets.targets["TMS1"].color)
check("the table row renamed", win.tgt_table.item(row, 0).text() == "Hercules")
check("the tie-in menu entry renamed",
      "Hercules" in win.act_tie["ROV1"].text(), win.act_tie["ROV1"].text())

# The whole point: nothing structural moved.
check("the slot itself never changed", "ROV1" in win.view.targets.targets,
      str(sorted(win.view.targets.targets)))
check("the tie-in still points at the slot",
      win.calib.points[0].vehicle == tied_slot == "ROV1")
...
check("the tether pairing is untouched", TETHERS["ROV1"] == "TMS1")
win.view.targets.draw_links(TETHERS)
pump(100)
check("so the tether still draws",
      win.view.plotter.renderer.actors.get("link:ROV1") is not None)

# A body renamed after the trail started keeps its trail.
check("the trail survived the rename",
      len(win.view.targets.targets["ROV1"].trail) > 0,
      str(len(win.view.targets.targets["ROV1"].trail)))
...
check("a restart brings back name and colour",
      reloaded.label("ROV1") == "Hercules"
      and reloaded.colour("ROV1") == "#00a3ff"
      and reloaded.colour("TMS1") == vehicles.darken("#00a3ff"))
...
check("and every colour", win.fleet.colour("ROV1") == "#ff3b30")
check("the table went back too", win.tgt_table.item(row, 0).text() == "ROV1")
check("and the tie-in point still survives",
      len(win.calib.points) == 1 and win.calib.points[0].vehicle == "ROV1")
